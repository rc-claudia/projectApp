...
/**
 * Created by Claudia on 06/01/2017.
 */
...
public class DBHandle extends SQLiteOpenHelper{
        ...
        public void addUser(User user) {
                ...
                boolean USER_NOT_ADDED = !doesUserExist(user.getUserOnScreen());

                if (USER_NOT_ADDED) {
                        ContentValues values = new ContentValues();
                        // Primary key is auto-generated
                        values.put(KEY_USERNAME, user.getUserOnScreen()); // Username
                        ...
                        Log.d(TAG, "addUser: " + user.getUserOnScreen() + " already exists in the database.");
                }
        }
        ...
        public ArrayList<User> getAllUsers() {
                ...
                if (cursor.moveToFirst()) {
                        do {
                                User user = new User();
                                user.setUserOnScreen(cursor.getString(1));
        ...
        public int updateUser(User user) {
                ...
                ContentValues values = new ContentValues();
                values.put(KEY_USERNAME, user.getUserOnScreen());
                values.put(KEY_EMAIL, user.getEmail());
                values.put(KEY_PASSWORD, user.getPassword());

                // updating row
                return db.update(TABLE_USERS, values, KEY_USERNAME + " = ?",
                        new String[]{user.getUserOnScreen()});
        }


        // Adding new attempt
        public void addAttempt(Score newGame) {
                SQLiteDatabase db = this.getWritableDatabase();

                boolean IS_VALID_ATTEMPT = doesUserExist(newGame.getUserOnScreen());

                if (IS_VALID_ATTEMPT) {
                        ContentValues values = new ContentValues();
                        values.put(KEY_USERNAME, newGame.getUserOnScreen()); // Username
                        values.put(KEY_SCORE, newGame.getScore()); // User score
        ...
        public int getScore(String username) {
                SQLiteDatabase db = this.getReadableDatabase();
                User user = new User();
                String thisUser = user.getUserOnScreen();

                String selectQuery = "SELECT score FROM WHERE username = thisUser" + TABLE_ATTEMPTS;
                Cursor cursor = db.rawQuery(selectQuery, null);
                ...
                if (cursor != null && cursor.getCount()>0) {
                        cursor.moveToFirst();
                        scoreToScreen = cursor.getInt(2);
                        cursor.close();
                }

                // return shop
                return scoreToScreen;
        }

        // Getting All Attempts
        public ArrayList<Score> getAllAttempts() {
                ArrayList<Score> gameList = new ArrayList<>();
                // Select All Query
                ...
                if (cursor.moveToFirst()) {
                        do {
                                Score thisGame = new Score();
                                thisGame.setUserOnScreen(cursor.getString(1));
                                thisGame.setScore(cursor.getInt(2));
                                // Adding contact to list
